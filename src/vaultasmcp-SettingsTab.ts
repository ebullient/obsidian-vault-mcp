...
export class VaultAsMCPSettingsTab extends PluginSettingTab {
    ...
    // ----------------------------------------------------------------
    // Shared rendering helpers (used by both display() and render callbacks)
    // ----------------------------------------------------------------
    ...
    // ----------------------------------------------------------------
    // < 1.13.0 fallback: imperative display()
    // ----------------------------------------------------------------
    ...
    display(): void {
                ...
                text
                    ...
                    .onChange(async (value) => {
                        ...
                        if (!Number.isNaN(port) && port > 0 && port < 65536) {
                            const prev = s.serverPort;
                            s.serverPort = port;
                            await this.saveAndMaybeRestart(prev, s.serverHost);
                        }
                    }),
            );
        ...
        new Setting(containerEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(s.autoStart).onChange(async (value) => {
                    s.autoStart = value;
                    await this.plugin.saveSettings();
                }),
            );
        ...
        new Setting(containerEl)
            ...
            .addButton((button) =>
                button.setButtonText("Test patterns").onClick(() => {
                    new PathACLTestModal(this.app, s.pathACL).open();
                }),
            );
        ...
        new Setting(containerEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(s.debug).onChange(async (value) => {
                    s.debug = value;
                    await this.plugin.saveSettings();
                }),
            );
        ...
        new Setting(containerEl)
            ...
            .addDropdown((dropdown) =>
                dropdown
                    ...
                    .setValue(s.serverHost)
                    .onChange(async (value) => {
                        if (value === "0.0.0.0" && !s.bearerToken) {
                            new Notice(
                                "⚠️ Network access without authentication is unsafe. " +
                                    "Generate a bearer token first.",
                                5000,
                        ...
                        const prev = s.serverHost;
                        s.serverHost = value;
                        await this.saveAndMaybeRestart(s.serverPort, prev);
                    }),
            );
        ...
        new Setting(containerEl)
            ...
            .addText((text) => {
                text.setPlaceholder("Leave empty to disable")
                    .setValue(s.bearerToken ?? "")
                    .onChange(async (value) => {
                        s.bearerToken = value.trim() || undefined;
                        await this.plugin.saveSettings();
                    });
        ...
        // MCP tools
        const toolsSetting = new Setting(containerEl).setName(
            "Available MCP tools",
        );
        const ul = toolsSetting.descEl.createEl("ul");
        for (const tool of new MCPTools(
            this.app,
            this.plugin,
            this.plugin,
        ).getToolDefinitions()) {
        ...
        new Setting(containerEl).descEl
            .createEl("a", {
                href: "https://www.buymeacoffee.com/ebullient",
                attr: { target: "_blank" },
            })
